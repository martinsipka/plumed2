...
class Plumed(Calculator):
    ...
    def calculate(self, atoms=None, properties=['energy', 'forces'], system_changes=all_changes):
        ...
        cvs1, grads1 = cv1(atoms)
        cvs2, grads2 = cv2(atoms)
        
        cvs = [cvs1, cvs2]
        grads = [grads1, grads2]
        
        
        energy, forces = self.compute_energy_and_forces(self.atoms.get_positions(), self.istep, cvs, grads)
        self.istep += 1
        self.results['energy'], self. results['forces'] = energy, forces

    def compute_energy_and_forces(self, pos, istep, cvs, grads):
        if world.rank == 0:
    ...
    def compute_bias(self, pos, istep, cvs, grads):
        ...
        self.cv1 = np.array(cvs[0], dtype=np.float64, copy=True)
        self.grad1 = np.array(grads[0], dtype=np.float64, copy=True)
        self.plumed.cmd("setExtraCV cv1", self.cv1)
        self.plumed.cmd("setExtraCVDerivative cv1", self.grad1)
       
        #Now pass the second value to the plumed
        self.cv2 = np.array(cvs[1], dtype=np.float64, copy=True)
        self.grad2 = np.array(grads[1], dtype=np.float64, copy=True)
        self.plumed.cmd("setExtraCV cv2", self.cv2)
        self.plumed.cmd("setExtraCVDerivative cv2", self.grad2)
